...
### Seeded 526 Indian foods from IFCT 2017 — and caught a 4.2x unit trap on the way in
*2026-08-02 · Feature*

We added the Indian Food Composition Tables (ICMR-NIN, 542 foods lab-measured across six regions) to the food cache, via the MIT-licensed `@ifct2017/compositions` package. The reason it's worth having alongside the 1,014 INDB rows we already had is that the two are different kinds of data: INDB is cooked *dishes* (poha, dosa, biryani), IFCT is raw *ingredient* composition — and crucially it carries the names Indian users actually type. Bajra, jowar, ragi, rajmah and paneer are primary entries in IFCT and resolve to nothing useful in USDA.

Two things nearly went wrong. First, IFCT publishes energy in **kilojoules**, not kilocalories — bajra reads 1456, which is 348 kcal. Seeding that column as-is would have made every Indian staple 4.2x too high, and it would have looked plausible enough in the database to go unnoticed for a long time. We checked the unit against the raw file before writing a line of the seeder. Second, and more subtly: 534 of the 542 rows are *raw* ingredients, and we spent this morning fixing a bug where cooked dishes wrongly matched raw database entries ("a bowl of rajma" hitting dry kidney beans at 337 kcal/100g, 2.6x too high). Bulk-loading 526 raw rows risked re-creating that at scale. So the seeder appends an explicit "(raw)" to the name for the ten food groups that are essentially always cooked before eating — cereals, legumes, meats, fish, roots and tubers, eggs — which turns the hazard into an asset, because the raw-vs-cooked rule we added to the matcher this morning can now *see* the word and reject those candidates for a cooked dish. 280 of the 526 rows carry it.

Honest result: the Indian category held at 14/20, exactly where it was before. No regression — which was the actual risk, and the raw-labelling defence is why — but no measured gain either, because every Indian case in our eval is a cooked dish and IFCT is deliberately not competing for those. Its value lands on ingredient-level logging ("150g paneer", "a handful of cashews", "a bowl of curd" — all passing), which our dataset barely covers. We kept it: 526 grounded rows at zero ongoing cost, and the cache is now 526 IFCT / 1,014 INDB / 127 USDA / 126 AI-guessed, meaning far more of what users log resolves to measured data rather than a model's invention. Also needed migration 024 to extend the `source` CHECK constraint, which rejected 'ifct' outright — a good reminder that the database's own guardrails catch what code review might not.

### Bumped the onboarding chat bubble up a step in the type scale
*2026-08-02 · Feature*

Shivam started a pass over the onboarding flow and asked for the typewriter greeting on screen 1 — the "Hey! I'm STEADY 👋" card above the goal options — to be a point bigger. The interesting part was finding where that font size actually lives: the typewriter effect is its own component (`TypewriterText`), but it deliberately owns no styling at all — it just spreads whatever `style` prop it's handed onto a plain `<Text>` and reveals the words on a timer. So the size was never in the animation code; it was in `ChatBubble`'s `bubbleText` style, one level up. Good reminder that "make the animated text bigger" and "find the file with the animation in it" are not the same search.

We moved it from `typography.sm` (12) to `typography.md` (14) rather than hardcoding 13, because STEADY's type scale has no 13 and inventing an off-scale number is exactly how a design system starts drifting — `md` is the next real step. Line height went 19 → 21 alongside it, keeping roughly the same 1.5 ratio so the two-line greeting doesn't feel cramped at the larger size. Worth flagging that `ChatBubble` is shared by all six onboarding screens by design (its own comment says so — one canonical look so padding and shadows never drift), so this bumps every bubble in the flow, not just screen 1.

### We turned down a free 4-million-product database, and showed grams on the meal card
*2026-08-02 · Decision*

Shivam wanted to seed more food data, and was willing to accept Open Food Facts' ODbL licence — which requires that if you combine their data with your own database, you publish the *combined* database as open data too. That's a real business decision for a commercial app, so rather than take the instruction at face value we tested his actual condition ("if this really makes our data best"). It doesn't. Five of six Open Food Facts API queries returned HTTP 503 even spaced six seconds apart; the one that answered matched "maggi noodles" to *Fromage Blanc Nature* out of 4.6 million hits; and only two of five returned products carried calorie data at all. The deeper problem is structural: Open Food Facts is a barcode database for packaged goods, and the foods we actually fail on are restaurant items (zinger burger, Starbucks cappuccino) and home-cooked dishes (masala dosa, rajma). Paying for that with our entire food database made no sense, so we skipped it.

What we found instead was free and already half-wired: USDA publishes ~1.9M manufacturer-submitted **Branded** products under CC0, and we had simply never been searching that tier — only Foundation, SR Legacy and FNDDS. Searching "maggi noodles" against Branded returns 3,217 real hits where the other tiers return none. We added it as a deliberate *third* tier rather than mixing it in, because branded descriptions are noisy marketing text ("amul butter" surfaces "BUTTER BALLS, BUTTER") and would beat good generic matches if allowed to compete. Reaching tier 3 means the curated tiers found nothing, where a noisy real product still beats the model inventing numbers. The branded strict score held at 11/15 so we're not claiming a headline win, but the composition improved: Starbucks cappuccino went from a 2.18x miss to a pass, and cappuccino, oreo, lays and snickers all moved off AI guesses onto real product data — which matters for consistency even where the band didn't flip, since ungrounded foods are also the ones that vary between runs.

We also fixed something Shivam spotted in a screenshot of his own log: the meal card showed "Masala Dosa (1 masala dosa)" with no gram weight anywhere. The card was rendering the quantity label *or* the grams, never both — so any count-style label hid the number entirely. That's precisely backwards, because grams are simultaneously the value most likely to be wrong and the only one worth correcting, since every macro is derived from it. Now it reads "1 masala dosa · 200 g", while labels that already state a weight ("small bowl ~60g") are left alone instead of being repeated back. Fittingly, that same screenshot was a live instance of the masala-dosa bug we'd fixed hours earlier — 140 calories, which is the *plain* 85g dosa weight rather than the ~200g stuffed one.

### 78.9% — and we tested whether the AI knows when it's wrong (it doesn't)
*2026-08-02 · Decision*

We closed out the remaining failure cluster and found it was one shared cause, not several. "A plate of chicken biryani" and "a plate of rajma chawal" were both landing ~40-45% short because our gram hints sized every plate at 200g. That's right for a single dish, badly wrong for a rice-based main course — a real biryani or rajma-chawal plate is 350-450g. Our earlier proportional-split fix had correctly stopped each component from claiming the whole plate's weight, but it was splitting a plate that was too small to start with. Adding a distinct "full main-course plate" size took us to **78.9% strict, 82.8% calories-in-band, median error 12.7%** — up from a 67.4% baseline at the start of the day.

Then we tested Shivam's idea: when the AI can't pin down a quantity, have it ask the user a follow-up question in the chat instead of guessing. Architecturally it's very doable — but the whole feature rests on one assumption, that we can tell *when* the AI is unsure, so we measured that instead of assuming it. The parse step already emits a per-food `confidence`, and it turns out to be directionally real: meals at 0.80 confidence had a 47% median calorie error, meals at 0.95 had 3%. But the model only ever emits four values and never goes below 0.80, with 86% of meals bunched at 0.85-0.90. As a trigger it collapses — a <0.80 threshold fires on literally zero meals, and <0.85 interrupts 6 users to catch 2 errors while staying quiet on 16 of the 18 real failures. We then tried deterministic triggers instead of the model's self-assessment (does the meal contain an ungrounded AI-guessed food? is the quantity wording vague?) and those were no better: the combined rule fires on 45% of all meals and still catches only half the errors — a coin flip that would interrupt nearly every other log.

So we're not building the ask-the-user-when-unsure loop, and the reason is worth writing down: **we can't predict which logs are wrong at the moment we log them.** The far better shape for the same instinct is correction rather than interrogation — and STEADY already has most of it. `editMealFromText()` sends the meal id plus its previous entries, and the deterministic rescale guard we built earlier means "actually that was a small bowl" re-scales the existing food instead of re-deriving it. The conversational chain Shivam was imagining exists; it's just user-initiated. The work worth doing is making portions visible and one-tap adjustable on the meal card so people can correct the ~1-in-5 that lands wrong — which is exactly the pattern MyFitnessPal settled on, and they report 1 in 4 photo logs needing a manual fix.

### 100 test cases, three root causes, and food logging goes from 67% to 74%
*2026-08-02 · Milestone*

We built a fresh 100-case eval — 95 real food messages across seven categories (Indian dishes, branded fast food, western home cooking, drinks with additives, raw items, explicit weights, and deliberately vague phrasing) plus 5 messages that must NOT create a log. Two methodology fixes mattered more than the extra cases. First, every case now runs on its own date, so chat history can't leak between them; our previous eval was accidentally measuring the history-saturation bug and macro accuracy at the same time. Second, we stopped requiring all four macros to sit inside hand-drawn bands and started reporting what the literature reports — calories in band, calories within ±25%, and median error. The old metric had been reading 42% on runs where the standard measure read 58-64%.

Baseline came in at 67.4% strict with a median calorie error of 15.3%, and the failures split cleanly into three genuinely different root causes rather than one. **Raw vs cooked**: "a bowl of rajma" matched USDA's *dry* kidney beans at 337 kcal/100g, then had a cooked-portion gram weight applied on top — 2.6x too high. The dietary-assessment literature names this exact trap ("application of raw ingredient composition may lead to overestimation") and its standard tool is yield factors, so we taught the matcher them directly: dry legumes and rice absorb ~2.5x their weight, meat loses ~25%. We also found "a can of red bull" resolving to the *sugar-free* variant at 5 kcal/100g, so the matcher now rejects diet variants unless the user actually said "diet" or "zero". **Portion weights**: our gram hints listed "dosa 85g" — correct for a plain dosa, badly wrong for a potato-stuffed masala dosa, and we had no per-piece weights at all for count-based items, so "6 chicken mcnuggets" became 180g instead of 96g. **Intent**: six clear food messages came back as chat replies, and every one of them was a bare noun phrase — "a plate of rajma chawal", "chicken curry with rice" — while all six of our few-shot examples were verb-led ("I had 2 eggs and toast"). The intent-classification literature is blunt about this: demonstrations semantically closer to the real queries produce the biggest gains. We added bare-noun-phrase examples.

That took us to 71.6%, and the remaining failures pointed at one dominant lever we'd flagged earlier but never fixed: additives in drinks. "Coffee with milk" was logging a full 240g glass of milk *plus* 240g of coffee — 3.6x over — and "2 spoons of sugar" became 30g when two teaspoons is nearer 10g. The reason our earlier fix never fired is that it lived in the match step, which only runs for cache misses; milk is AI-estimated and has no USDA portions to correct against. The right home was the parse prompt, which runs for every food regardless of cache state. Final: **73.7% strict, 76.6% calories-in-band, median error 13.3%, 5/5 on not-logging non-food, zero errors** — up from 67.4% with intent misses down from 6 to 1. Full Jest suite still 180 passing. Worth noting honestly: the ±25% metric wobbled 63% → 61% between the last two runs while the other three metrics rose, which is the LLM run-to-run variance we already documented — single-run deltas under a few points aren't signal.

### We found a broken URL parameter that had been quietly gutting our nutrition data
*2026-08-02 · Bug*

Chasing why our food-logging accuracy sat at ~40%, we went to the research literature first instead of guessing. The most relevant benchmark (NutriBench, 2024) tests exactly our task — macros from casual meal descriptions — and its headline finding matched our own failure data precisely: *"the primary source of estimation error is not nutrient composition, but portion size."* Their best result, GPT-4o with chain-of-thought, is 66.8%. Even MyFitnessPal, with a 14-million-food database, reports ±18% portion error and needs manual correction on 1 in 4 photo logs. That reframed the goal: we weren't chasing a bug that would take us to 95%, we were chasing a known-hard problem where the state of the art is ~67%.

Then, while checking whether USDA could supply portion reference data, we found something much more concrete. Our `usda.ts` built its fallback search by joining data types into one comma-separated string containing `Survey (FNDDS)` — parentheses and all. That request fails with a 400 about two times in three (we measured it: 2/6 success comma-joined, 6/6 with repeated `dataType` params). And FNDDS is the *only* USDA tier covering prepared and composite dishes. So every samosa, biryani, latte and vada pav would miss tier 1 (raw ingredients only), 400 on tier 2, get flagged as "USDA is down", fall through to a pure LLM guess — and because the outage flag also disables caching, never get cached, so every repeat log re-rolled a fresh guess. A good chunk of what we'd previously written off as "temperature-0 nondeterminism" was actually this. One-line fix; USDA went from a minority source to the dominant one.

The bonus was that the same fixed response carries `foodMeasures` — USDA's own household-measure table ("1 piece = 30g" for sushi, "1 item = 38g" for idli). That's exactly the portion reference we'd been planning to hand-write, except sourced from USDA rather than guessed. We wired it in two ways: the LLM match step now sees the published measures and can revise the parse step's gram estimate, and we added a `portions` column (migration 023) plus a backfill script so foods served straight from cache — which deliberately skip the LLM entirely — get a free, deterministic correction too. The proof it works: "a bowl of cereal" went from 150g (a bowl-of-cooked-*rice* weight) to 21g, dropping that meal from 737 calories to 230. Nine unit tests cover the conversion logic, including the trap that USDA publishes cereal as "1.5 cup = 32g", so you have to divide the amount back out before scaling.

Honest status: the headline eval number hasn't moved yet (~42% on our strict all-four-macros-in-band test, ~58-64% on the calories-within-25% measure the literature actually uses). The correction only fires for foods USDA publishes measures for, and the biggest remaining offender is milk — logged as a full 240g glass when someone says "coffee with milk", where it should be a ~30g splash. That's the next thread to pull.

### Old medicines saved before `days` existed crashed the Medicine screen
*2026-08-02 · Bug*

`Cannot read property 'length' of undefined` on opening Medicine — caused by any medicine added before today's day-selection feature. `config` is stored as raw JSONB with zero schema enforcement, so adding `days: string[]` to the `Medicine` TypeScript type only changed what *new* saves look like; every medicine already sitting in a user's `notification_preferences` row from before this session simply doesn't have that key in its JSON, and `fetchReminders()` was casting that JSON straight to `ReminderConfig` with no validation (`row.config as unknown as ReminderConfig`), so `undefined` flowed straight into `med.days.length` on render. Fixed with a `normalizeConfig()` step that runs once, right after a config comes back from Supabase, backfilling `days: []` onto any medicine missing it — one choke point instead of null-guarding every read site in the UI. General lesson for this app: a TypeScript type change is not a data migration. Any time an existing JSONB-shaped field gains a new key, old rows need either a real SQL backfill or a normalization pass on read, because the type checker has no way to see what's actually sitting in the database.

### Opening a reminder now enables it immediately, but only actually turns on if you save
*2026-08-02 · Fix*

Tapping "Set" on an off reminder used to drop you into a form that was still off — the header toggle, the whole config card grayed out and non-interactive (`pointerEvents: 'none'`), forcing an extra manual tap before you could even set a time. Fixed by seeding the screen's local `enabled` state to always start `true` on open, so the form is immediately editable regardless of whether the underlying reminder was on or off. The other half of this — not persisting that "on" state if you back out without saving — turned out to already be correct by construction: `ReminderDetailScreen` only ever talks to the store/DB from inside `handleSave`, and the back button (header chevron, hardware back) never calls it, so backing out just discards the local draft entirely and the reminder stays exactly as it was before you opened the screen. Worth remembering as the general shape for edit screens in this app: keep everything as local component state until an explicit Save, and "cancel" becomes free — there's nothing to undo because nothing was ever written.

### Medicine reminders can now pick specific days, not just every day
*2026-08-02 · Feature*

Real-world medicine schedules aren't always daily — a supplement might be every day, but a course of antibiotics might be Mon/Wed/Fri, and a single reminder can hold several medicines each on its own schedule. Added a `days: string[]` field to each `Medicine` (empty array means every day, so nothing existing breaks), plus a row of 7 tappable weekday chips under each medicine in `MedicineBody`, reusing the same visual pattern already established for weekly recurrence elsewhere on this screen. The trickier half was making this actually fire correctly, not just look right in the UI: `find_due_reminders()` already had a recurrence gate from migration 021, but that one checks a single `freq`/`day` at the top of a reminder's whole config — fine for weight/health-log (one schedule per reminder), useless for medicine (multiple independent per-item schedules in one list). Migration 022 adds a second, medicine-specific gate that reaches into `config->'meds'`, finds whichever medicine's `time` matches the row currently being checked, and only lets it through if that specific medicine's `days` is empty or contains today's weekday — so two medicines with different day patterns coexist correctly in the same reminder.

### Medicine reminders had no way to actually name a medicine, and the time tap looked dead
*2026-08-02 · Bug*

Two real gaps in `MedicineBody`, not one. The name was rendered as a plain `<Text>` with the hardcoded string `'New Medicine'` inserted on Add — there was never a `TextInput` anywhere in the component, so there was no code path that could have let anyone rename it. The time control was technically wired to `openTime()` correctly, but it was a bare, unstyled text line (`med.dose · time`) with zero visual affordance that it was tappable — no pill, no background, no chevron — so from the user's side it read as static text even though the `onPress` fired. Fixed both: name and dose are now real `TextInput`s (each `padding: 0`, since RN's default TextInput padding would've knocked their baseline out of alignment with the sibling `Text` elements around them), and the time now uses the same `TimePill` component every other reminder type in this screen already uses, so it looks and behaves consistently instead of like inert text.

### Fixed the Reminders list icons back to rounded squares, taller rows
*2026-08-02 · Bug*

Our port of the Reminders list drifted from the Claude Design mockup in one detail: the per-habit icon container was a full circle (`borderRadius: 18` on a 36px box — radius equals half the width, which is the geometric definition of a circle) instead of the rounded square the design actually used (`borderRadius: 13` on a 44px box). Renamed the style from `iconCircle` to `iconBox` to stop the name itself from suggesting the wrong shape, bumped the icon back up to 44px to match, and grew each row's height (54px to 66px, more vertical padding) so the cards read a little less cramped with the bigger icons in place.

### Found and fixed a bug where STEADY silently stops logging food for active users
*2026-08-02 · Bug*

We built a second, more realistic eval — 50 test cases written as an actual user would text them ("just had a bowl of dal and rice", not "1 bowl dal, 250g"), with ground-truth macros sourced from real web nutrition data instead of our own guesses, run against the real deployed edge function (not the resolver in isolation) using a dedicated test account signed in exactly like the app does. First run: only 9/50 passed, and the failure pattern was strange — 36 of 45 food-logging messages came back classified as "answer" instead of "log", even dead-simple ones like "a can of coke".

Root cause: `log-food-from-text` replays up to 20 prior chat messages for context on every call, and every successful food log gets replayed back as an ordinary assistant reply — same shape, same tone as a coaching answer. After ~10 exchanges in one sitting (very reachable for a real user: breakfast, snack, lunch, a question, dinner...), the model started pattern-matching "this whole conversation is just chat" and misclassified brand-new food logs as chat too — confirmed by reproducing the exact same message on a fresh, zero-history account (worked fine) versus the saturated one (failed every time). Three prompt-wording fixes in a row didn't touch it — the sheer volume of repeated "log → prose reply" shape in the history was winning against a written instruction every time. The fix that actually worked: stop replaying food logs as individual prose turns at all. Coaching Q&A stays as real conversational messages (that content matters for follow-ups); food logs collapse into one compact "already logged N meals today" system note instead of N separate replies — no more repeated shape for the model to latch onto.

While validating the fix, the same 50-case eval also caught three real macro-accuracy bugs completely unrelated to history: a Starbucks latte was priced 700+ calories because milk-based coffee had no sanity anchor and got treated like a syrupy dessert drink; "2 pints of beer" only got the gram weight of one pint because "pint" wasn't in our unit-conversion reference; and mixed sushi resolved against raw-fish-only USDA data, silently dropping the rice and reporting 0g carbs for something that's mostly rice. Fixed all three with targeted sanity-anchor and unit additions. Along the way also hardened the resolver itself: it used to hard-crash an entire meal log if the AI match step failed to return either a matched candidate or a fallback estimate for even one ingredient — now it falls back to a generic estimate for just that one food (and explicitly never caches that guess, so it can't poison future logs) instead of losing the whole log. Final clean-account re-run: intent misclassification down from 36/45 to 2/45, and the coffee/beer/sushi fixes confirmed by direct database checks. Also surfaced something worth remembering going forward: even at temperature 0, the same exact message can still get meaningfully different gram estimates from OpenRouter run to run (a "4-5 slices of pizza" parse came back as 1000g total on one run, ~700g on another) — "temperature 0" reduces variance, it doesn't guarantee it away.

### Save reminder button did nothing — the migration was written but never actually pushed
*2026-08-02 · Bug*

Shivam reported the new Reminders detail screen's Save button just sat there doing nothing on tap. The cause: migration 021 (the one adding the `config` JSONB column the new store relies on) existed as a file in the repo, but we'd never run `supabase db push` against the live database after writing it — `supabase migration list` showed it present locally and completely absent on the remote side. Every save was quietly failing with a "column not found" error from Supabase, caught by our own `try/catch`, which meant `navigation.goBack()` never ran and nothing visible happened. Pushing it surfaced a second, real issue: `CREATE OR REPLACE FUNCTION` refused to touch `find_due_reminders()` with `SQLSTATE 42P13` ("cannot change return type of existing function"), even though the `RETURNS TABLE` shape was logically identical to migration 013's — Postgres checks this structurally at the catalog level, not by comparing SQL text, so a byte-for-byte-looking signature can still fail the check. Swapped it for `DROP FUNCTION IF EXISTS` followed by a plain `CREATE FUNCTION`, which sidesteps that check entirely, and the push went through clean. Both migrations run inside one transaction per file, so the first failed attempt left nothing half-applied to clean up.

### Rebuilt Reminders from a generic time-picker into seven real, tailored schedules
*2026-08-02 · Feature*

The old Reminders screen treated every habit the same way — toggle it on, then set a list of clock times in one generic drum-picker sheet. That's fine for "workout at 7:30 PM," but it can't honestly represent "which 3 of my 5 meals am I tracking," "every hour vs. 3 specific times for water," "weekly on Tuesday vs. monthly on the 25th" for weight/health-log, or an open-ended list of named medicines with their own doses. We had already designed the real thing in Claude Design (`steady-screens-reminders.jsx`) months ago but never ported it into the app, so today we finally built it: `reminderStore.ts` now models each reminder's schedule as a TypeScript discriminated union (`DailyConfig | MealsConfig | WaterConfig | RecurringConfig | MedicineConfig`), and a new full-screen `ReminderDetailScreen` renders a different body component per type — a meal checklist, a water window-and-mode picker with a live time list, weekday chips or a day-of-month stepper for recurring reminders, and an add/remove medicine list. The old cramped bottom-sheet flow is gone; tapping Edit/Set now pushes a real screen with an icon, a one-line explainer of the habit, the config card, and a sticky Save bar, matching the design exactly.

On the backend, `notification_preferences` gained a `config JSONB` column (migration 021) to carry all that type-specific data, since the old flat `times TEXT[]` genuinely couldn't hold it. `find_due_reminders()` got one new gate: for weight/health-log reminders it now also checks `config->>'freq'` against the current weekday or day-of-month before firing, so "every Sunday" and "monthly on the 25th" are respected server-side, not just displayed correctly in the UI. Every other reminder type still flows through the exact same window-matching SQL from migration 013, and `send-scheduled-notifications`'s Edge Function needed zero changes — it only ever calls the RPC and never touches `times`/`config` directly, which is exactly the kind of boundary that made this safe to extend without touching the scheduler.

### Built a macro-accuracy eval harness after catching pizza logging 22g protein for 560 calories
*2026-08-02 · Bug*

Shivam logged "two slices of pizza" and got back numbers that don't add up — 22g protein at 560 calories implies a protein density closer to bread than to cheese pizza. We went digging in the macro-resolution pipeline (`macroResolver.ts`) built back on 2026-07-02 specifically to stop the AI from inventing nutrition numbers, and confirmed the architecture itself is sound: food names + gram estimates come from one AI parse call, then macros are resolved deterministically through a cache → USDA → one-time-AI-estimate chain, with the arithmetic done in plain code, never by the model. The likely culprit isn't the architecture — it's that "pizza" isn't in the seeded Indian Nutrient Databank (that seed is India-focused) and may not match cleanly against USDA's raw-ingredient search, so it could be falling through to the AI-estimate fallback tier, which — if wrong once — gets cached forever and silently wrongs every future pizza log.

Rather than guess, we built an eval: a Node/TypeScript script (`scripts/eval/run.ts`) that runs the exact production pipeline — the real parse prompt against OpenRouter, then the real `resolveFoods()` against the live Supabase cache and live USDA API — over a curated set of 30 food descriptions with independently-sourced ground-truth macro ranges (pizza, burgers, Indian dishes that should hit the INDB cache, raw ingredients, packaged foods). It reports a pass rate plus a breakdown by category and by `macro_source`, so a systemic bug in one resolution tier (e.g. "AI-estimate is bad for Western composite dishes") shows up as a pattern instead of one anecdote. One small production refactor came out of this: `SYSTEM_PROMPT` moved from inline in `log-food-from-text/index.ts` into a new shared `_shared/foodParsePrompt.ts`, so the eval calls the identical prompt production uses instead of maintaining a copy that could drift.

### Ran the eval, found 3 real bugs, fixed all of them, and cleared up the actual pizza mystery
*2026-08-02 · Bug*

First run against production: 15/30 passed (50%). Digging into the misses split cleanly into "my ground-truth ranges were too strict" (several Indian dishes are legitimately more oil-rich than a health-conscious guess assumes) versus three actual, reproducible bugs. First: the fuzzy-match step was letting "apple" match against INDB's "Apple mousse"/"Apple cake" — same headword, completely different food (a raw fruit vs. a dessert with added cream and sugar) — because the matching prompt never explicitly called out "ingredient name inside a dish name isn't the same food." Second, and the more consequential one: composite dishes broken into components (the app's own rule — "break compound foods into components") were getting the *whole dish's* portion weight applied to *each* component separately. "A plate of chicken biryani" became chicken 200g + rice 200g + spices 50g — 450g total for what should've been one ~250g plate — because the gram-estimation hints only covered "1 plate of a cooked dish ≈ 200g" for a single food, never for a dish split across several. Third, once that was fixed and re-tested, a new one surfaced: the AI-estimate fallback priced "butter sauce" at 900 kcal/100g and literally 100g of fat per 100g — physically impossible for a sauce (that's the "pure oil" sanity anchor being wrongly applied to a diluted, dairy-and-tomato-based gravy) — so we added a dedicated sauce/gravy anchor band (100–250 kcal/100g, richest ones up to ~350) to stop that category from defaulting to "oil."

Each fix got confirmed by clearing just the poisoned cache rows it affected and re-running only those examples (added a `--ids=` filter to the eval script for this — cheaper than a full 30-example re-run every time) rather than trusting the fix blind. All three landed clean: apple, paneer butter masala, and the gram-split all pass now. Original pizza case turned out fine on inspection — it resolved via USDA to a reasonable 414 kcal / 16g protein for two slices, just outside our conservative expected range, not the systemic bug we went in expecting. Also found and ruled out a fourth suspect: INDB's "Poha" row looked wrong at first (294.5 kcal/100g felt high), but its calories check out exactly against its own protein/carbs/fat via the standard 4/4/9 rule — it's just a genuinely oil-heavier recipe than a lean home-style poha, the same kind of real-world variance "a slice of pizza" has. Lesson for the app: single-number "ground truth" doesn't really exist for home-cooked food — the eval's job is catching systemic errors (wrong food matched, wrong portion math, wrong sanity anchor), not chasing an exact calorie count that doesn't exist in reality.

### Fixed Groups skipping the Invite screen and a flickering dashboard card
*2026-08-01 · Bug*

Shivam caught two real bugs on-device after creating his first group: tapping "Create group" jumped straight to the Dashboard instead of showing the Invite screen with the code, and the Dashboard's hero card was visibly glitchy — a rectangular artifact cutting across the gradient, content spilling outside the card's edges.

The navigation bug traced back to `GroupsIntroScreen`'s "already have a group, skip the pitch" redirect — it used a plain `useEffect` watching `myGroups`, which fires whenever the Zustand store updates *regardless of whether the screen is actually visible*. React Navigation keeps prior screens mounted (just unfocused) when you push forward, so `GroupsIntro` was still sitting in the background the whole time the user was on Create → Invite. The instant `createGroup()` updated the store, that background effect fired and called `navigation.replace('GroupHome')` — which replaces whatever's on top of the stack at that moment, yanking the user straight from Invite to the Dashboard before they ever saw the code. Fixed by swapping to `useFocusEffect`, which only runs while the screen is the one actually on screen — the same fix pattern worth remembering any time a screen needs to react to global state but only when it's the active view.

The gradient glitch was a different kind of bug: the hero card's background used an SVG `<Rect>` sized with `width="100%" height="100%"`, but percentage sizing on `react-native-svg` doesn't reliably resolve to the parent view's real pixel size on every platform — it can silently fall back to a small default canvas, which is exactly the "smaller box floating inside the bigger card" look in the screenshot. Fixed by measuring the card with `onLayout` and passing its exact measured pixel width/height to the `<Svg>` instead of a percentage string. Also found and fixed the underlying cause of the dashboard "fluctuating" while investigating: `GroupHomeScreen` was calling `fetchMyGroups()` (which always builds a brand-new array) on every single focus via `useFocusEffect`, not just on first load — any time focus returned to the screen it would refetch and produce a new array reference, triggering a visible re-render churn. Now `fetchMyGroups()` only runs once on mount and on explicit pull-to-refresh; focus-triggered refreshes are scoped to just the leaderboard/activity data that's actually meant to update live. `tsc --noEmit` clean, Jest still 171/172.

### Trimmed Groups down to STEADY's actual visual scale
*2026-08-01 · Bug*

First real on-device look at the Groups Intro screen and it was immediately obvious the sizing was off — 56px buttons, a 28px headline, 76px hero avatars, and long paragraph copy, none of which matches how the rest of the app actually looks. Traced it back to the source: the 5 screens were built faithfully off the Claude Design mockups, but that design's own type scale runs bigger than what STEADY settled into as it got built out (`WaterScreen`/`SignupScreen` use 46-48px buttons and 17-22px headers, not 56px/28px). Matching the *design file* wasn't the same as matching the *shipped app*, and nobody had checked the two against each other until now.

Went through all 5 screens plus the shared components (`GroupActivityScoreCard`, `LeaderboardRow`, `ActivityFeedItem`, `InviteCodeCard`) and pulled every size down to the app's real numbers: buttons to 46-48px, screen titles to 16-22px depending on hierarchy, icon wraps and avatars roughly 25-30% smaller, feature-list copy trimmed to single lines instead of two-line paragraphs. The Intro screen also switched from a scrolling layout to a fixed one that centers its content vertically — with the tighter sizes, everything now actually fits on one screen without scrolling, which was part of the ask. `tsc --noEmit` clean and Jest still 171/172 throughout — this was a pure styling pass, no logic touched.
*2026-08-01 · Feature*

With the database layer verified, we built the rest of Groups on top of it: a `groupStore.ts` (Zustand, following the same conventions as `weightStore`/`foodLogStore` — RPC calls for anything cross-table, optimistic updates for cheers), five reusable components (`GroupAvatar`, `LeaderboardRow`, `ActivityFeedItem`, `GroupActivityScoreCard`, `InviteCodeCard`), and all five screens from the original design — Groups Intro, Create, Invite, Join, and the Dashboard with its Leaderboard/Activity Feed tabs. The intro screen does a quiet redirect-on-mount to the dashboard if the user already has a group, so both entry points (the drawer's "Groups" row, now live instead of "coming soon," and a new icon in the Home header) can point at the same screen without duplicating the "does this user have a group" check.

One nice discovery along the way: instead of hand-typing the new tables into `database.ts` as originally planned, `supabase gen types typescript --linked` generates it straight from the live schema — we diffed the output against the existing file first to confirm it only *added* the new Groups tables/functions and touched nothing else, then swapped it in. Safer than hand-copying, and it's now the way we'll regenerate that file going forward. Also skipped `expo-linear-gradient` for the dashboard's hero card gradient — it isn't installed anywhere in this project — and used `react-native-svg`'s own gradient support instead, the same way `CalorieRing.tsx` already does it, so Groups didn't need a new native dependency (the only new package pulled in was `expo-clipboard`, for the invite-code copy button).

Since this is a genuinely new kind of feature (multi-user, real-time-ish social data), we leaned on the two-account SQL verification from the previous session rather than re-testing blind: the store calls the exact same RPCs, with the exact same parameter shapes, that were already confirmed correct against two real accounts. `tsc --noEmit` is clean and the full Jest suite is still 171/172 (1 skipped, unchanged baseline) after all of this. What's still open: seeing it actually render on a device — that's a `npx expo start` + Expo Go pass, next.
