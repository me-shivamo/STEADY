...
### Ripped the "Coming soon" stubs out of the live app, not just the docs
*2026-07-23 · Decision*

Follow-up to the docs/dead-code cleanup: Shivam asked whether we'd also removed the "Coming soon" placeholders from the actual running app, and the honest answer was no — the first pass only touched documentation and never-committed scaffolding. Grepping the real UI turned up 8 live, tappable stubs: seven rows in `ProfileDrawer.tsx`'s drawer menu (Progress Charts, My Foods, Reminders, Groups, Refer a Friend, Help & Support, Go Premium — all wired to a generic `Alert.alert('Coming soon'...)` handler) plus "Add to Saved Entries" in `MealCard.tsx`'s options sheet. These aren't harmless placeholders — a real user taps one in their first session and hits a dead end, which is exactly the kind of thing we'd already cut the Journal tab for earlier in the project. Removed all 8: trimmed `ProfileDrawer.tsx`'s `MENU` array down to the four rows that actually work (Weight, Water, Body Measurements, Settings) plus Sign Out, and deleted the "Add to Saved Entries" row from `MealCard.tsx` entirely. That left `MenuItem`'s `action` type carrying a dead `'comingSoon'` branch and `handlePress`'s fallback `else` unreachable, so we simplified both down to just `'signOut' | 'navigate'`. Removing "Go Premium" also meant `MenuRow.tsx`'s `'premium'` variant, `subtitle` prop, and `badge` prop had no remaining caller — trimmed those from the shared component too, along with their now-dead styles (`rowPremium`, `labelPremium`, `subtitle`, `badge`, `badgeText`), so the component's public API only claims to support what's actually used (`'default' | 'destructive'`). Ran `tsc --noEmit` after each change to confirm nothing broke; the only errors in the full project output are pre-existing and unrelated — Deno-runtime globals (`Deno.serve`, remote `esm.sh`/`deno.land` imports) in the Supabase edge functions, which plain `tsc` was never going to understand since those are typechecked by Deno's own toolchain, not this repo's config.

### Cleaned production branch down to only what's actually shipping
*2026-07-23 · Decision*

Shivam asked us to make the `production` branch reflect reality only — no aspirational docs, no unbuilt-feature scaffolding — while leaving `master` completely untouched, so we treated this as a documentation-and-dead-code audit rather than a feature change. First we grepped for every "AI Trainer/Coach/Nutritionist" mention across the repo and found the wording only ever showed up as "AI Nutritionist," and only in docs — `supabase/functions/chat-nutritionist/` turned out to be an empty, never-committed stub folder that nothing in the app calls. While auditing we found more unbuilt surface area than just that one feature: `README.md` and `ROADMAP.md` both described a much bigger product than what's live — barcode scanning, weekly progress charts, RevenueCat monetization, a Journal tab, Victory Native charts (never actually installed in `package.json`) — so after checking with Shivam we rewrote both to describe only the shipped app (photo/text AI logging, weight/water/measurements, onboarding, calorie ring, PostHog analytics). We also deleted the original 860-line product-spec plan (`.claude/plans/hey-claude-i-want-virtual-puddle.md`) and two more empty, never-committed stub folders we found along the way (`supabase/functions/revenuecat-webhook/`, plus three empty `src/components/` subfolders: `charts/`, `feed/`, `chat/`) — none of these were ever tracked in git history, so removing them had zero effect on git blame or `master`. Fixed one more thing in `CLAUDE.md` itself: its tech-stack line claimed "Claude claude-sonnet-4-6 (AI nutritionist)," which — per an earlier `LEARNING.md` entry — was already known to be factually wrong even before today's cleanup, since the edge functions only ever call OpenAI via OpenRouter. Deliberately left `DEVLOG.md` and `LEARNING.md` untouched everywhere else, since these are append-only history logs — the old mentions of Nutritionist/Coach are a record of what was *planned and decided*, not a claim about what's live today, and rewriting history would be dishonest in a different way than the docs drift we just fixed.

### Rewrote the legal pages after finding they described a different app than the one we shipped
*2026-07-23 · Bug*

While debugging the Play Store rejection we went digging through `steady-legal`'s existing privacy policy and terms, and found real factual drift between what the docs claimed and what the code actually does. Sent an Explore agent through the codebase to verify every claim before rewriting anything: turned out the privacy policy said "OpenRouter / OpenAI models" for AI processing, but the real chain (per `analyze-food-photo/index.ts` and `log-food-from-text/index.ts`) is app → OpenRouter → OpenAI's gpt-4o/gpt-4o-mini — Anthropic/Claude, despite being named in `CLAUDE.md`'s tech stack line, is never called anywhere in the shipped code. Also found PostHog's `identify()` calls send real name/email into analytics (`authStore.ts`), not just anonymous events like the old policy implied, and confirmed the onboarding age picker (`OnboardingStatsScreen.tsx`) accepts ages 10–100 with zero enforcement despite both legal docs claiming an 18+ requirement — a real mismatch between stated policy and actual product behavior. Rewrote `privacy.html` and `terms.html` with the corrected subprocessor chain, an explicit user-rights section (access/correction/deletion/export, CCPA and EEA language), retention specifics per data type, encryption-at-rest wording, and — new to terms.html — a liability cap and India governing-law clause modeled on Cal AI's public ToS. Kept the 18+ language in the final version per Shivan's call, but flagged that the onboarding age picker still doesn't enforce it — a real product gap, not just a docs gap, worth fixing separately.

### First Play Store submission bounced — a stray checkbox, not a code bug
*2026-07-22 · Bug*

STEADY's first upload came back rejected with "Violation of Play Console Requirements," which sounds like a code problem but is actually Google Play's account-tier policy: certain app categories (financial services, VPN, government, and specific health features) can only be published from an *Organization* developer account, not a personal one. Digging into the App content → Health apps questionnaire, we'd checked three boxes — Activity and fitness, Nutrition and weight management, and Period tracking. That last one doesn't correspond to anything STEADY actually does (no cycle-tracking feature exists anywhere in the codebase), and period tracking is one of the specific health features Google treats as high-sensitivity, so it was almost certainly the actual trigger. We unchecked it, left the two accurate boxes checked, and resubmitted — no code changes needed, just a corrected policy declaration.
...
### Seeded 90 days of realistic history into a screenshot account for the Play Store listing
*2026-07-22 · Setup*

Play Store listings need real-looking screenshots, and a brand-new account shows nothing but empty states — no weight trend, no meal history, no streak. Rather than fake it in a design tool, we seeded actual rows into Shivam's dedicated screenshot account through Supabase: 90 days of weight entries following a realistic trend (daily noise layered on a slow linear drift, ~15% of days skipped since nobody weighs in literally every day), body measurements every 10–14 days trending in step with the weight, and ~65 days of breakfast/lunch/dinner/snack history with matching chat-bubble exchanges so the "Log + Coach" feed doesn't look like bare cards. Wrote it as a one-off Node script using `@supabase/supabase-js` with the project's `service_role` key instead of the app's normal `anon` key — the anon key is bound by Row Level Security to only ever touch the signed-in user's own rows, and this needed to write another account's data directly. The script resolved the target user by email through the admin API first and refused to run if it didn't find exactly one match, then ran in a dry-run mode (print the plan, write nothing) before a second `--apply` pass actually inserted anything. Deliberately left `daily_summaries` alone — that table is maintained entirely by DB triggers (`003_triggers_functions.sql`) that recompute it from `food_entries`/`water_logs` on every insert, so hand-writing it would risk drifting out of sync; verified after the run that the trigger had in fact rolled everything up correctly. The service role key was only ever passed as an environment variable at invocation time, never written to a file in the repo, and the whole script lived under a newly gitignored `scripts/tmp/` before being deleted once the data was confirmed good.

### Turned the one-off screenshot seed into a proper, reusable demo-account script
*2026-07-26 · Feature*

Needed a second, smaller dataset — one week, not ninety days — for `demo@gmail.com` and `demo2@gmail.com`. Rather than write yet another disposable script, we extended `scripts/seed-demo-accounts.ts` (an existing, more polished version of the same idea, with a rotating catalogue of realistic meals instead of the earlier version's random food picker) so it covers everything the earlier one-off did: `water_logs`, `chat_messages`, and a computed `streaks` row, on top of its existing weight/measurements/meals seeding. Trimmed its measurement cadence from 3 points over 14 days down to 2 (start and today) since a week is too short for the usual ~10–14 day gap between real check-ins, and made every meal's `created_at` land at a believable hour (8am breakfast, 1pm lunch, etc.) instead of all defaulting to "now," so the Home screen's chronological "Log + Coach" feed reads in a sensible morning-to-night order. Ran it for both demo accounts and confirmed `daily_summaries` and `streaks` (current=7, longest=7 for both) came out correct. This is now the one canonical seed script in the repo — the throwaway `.mjs` version written for the first run was deleted rather than left alongside it.
